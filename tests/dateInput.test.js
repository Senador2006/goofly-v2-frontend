...
describe('formulários usam DateInput segmentado', () => {
  ...
  it('DateInput sanitiza dia/mês e reporta datas fora do intervalo', () => {
    ...
    assert.match(source, /date-input-picker/)
    assert.match(source, /type="date"/)
    assert.match(source, /Abrir calendário/)
    assert.match(source, /calendar_today/)
    assert.doesNotMatch(source, /showPicker/)
  })

  it('DateInput usa overlay nativo no ícone (sem showPicker / botão proxy)', () => {
    const source = readFileSync(join(root, 'src/components/common/DateInput.jsx'), 'utf8')
    const css = readFileSync(join(root, 'src/index.css'), 'utf8')
    assert.doesNotMatch(source, /<button[\s\S]*Abrir calendário/)
    assert.doesNotMatch(source, /h-px w-px/)
    assert.doesNotMatch(source, /pointer-events-none absolute bottom-0 right-0/)
    assert.match(source, /date-input-picker absolute inset-y-0 right-0/)
    assert.match(css, /\.date-input-picker::-webkit-calendar-picker-indicator/)
  })
})
